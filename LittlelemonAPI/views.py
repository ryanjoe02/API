...
from .serializers import MenuItemSerializer, CategorySerializer, UpdateCartSerializer, AddCartSerializer, CartSerializer, CreateOrderSerializer, UpdateOrderSerializer, OrderSerializer, SimpleOrderSerializer
...
class OrderViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.groups.filter(name="DeliveryCrew").exists():
            return (Order.objects.filter(delivery_crew=user.id)).select_related("user").all()
